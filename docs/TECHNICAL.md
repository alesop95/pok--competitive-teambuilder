...
Il meteo è offerto come opzione (`DamageOptions.weather` in `src/calc.ts`) e non più solo escluso: la baseline resta neutra, ma quando un team ha un weather setter tra i membri (rilevato da `src/pkmnData.ts:teamWeather`) la sua coverage offensiva viene calcolata sotto quel meteo, così una squadra pioggia vede le STAB Acqua potenziate (verificato: Basculegion Wave Crash vs Garganacl passa da circa 104% a 156% sotto pioggia). La *viability* per candidato resta invece neutra, perché è team-agnostica. Restano deliberatamente fuori dalla baseline l'oggetto (il difensore del meta ha item ignoto) e il modificatore 0.75 dei colpi multi-bersaglio, perché la domanda di coverage è a bersaglio singolo; sono scelte di baseline, non limiti nascosti.
